...
export const RegisterCategory = ({ onClose, dataSelect, action }) => {
  const { insertCategory, updateCategory } = CategoryStore();
  const { dataCompany } = CompanyStore();
  const [currentColor, setCurrentColor] = useState("#d70000");
  ...
  useEffect(() => {
    if (action === "Update" && dataSelect.color) {
    ...
    <Container>
      <div className="sub-contenedor">
        <div className="headers">
          <section>
            <h1>{action == "Update" ? "Update category" : "New category"}</h1>
          </section>
