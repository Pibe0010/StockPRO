...
const AllStock = () => {
  const { allReportStock } = ProductStore();
  const { dataCompany } = CompanyStore();

  const { data, isLoading, error } = useQuery({
    queryKey: ["all report stock", { id_company: dataCompany?.id }],
    queryFn: () => allReportStock({ id_company: dataCompany?.id }),
    enabled: !!dataCompany,
  });
          ...
          <Page size="A4" orientation="portrait">
            <View style={styles.page}>
              <View style={styles.section}>
                <Text style={styles.title}>All current stock</Text>
