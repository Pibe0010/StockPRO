...
const MinStockProduct = () => {
  const { reportMinStockProducts } = ProductStore();
  const { dataCompany } = CompanyStore();

  const { data, isLoading, error } = useQuery({
    queryKey: ["Min report stock product", { _id_company: dataCompany?.id }],
    queryFn: () => reportMinStockProducts({ _id_company: dataCompany?.id }),
    enabled: !!dataCompany,
  });
          ...
          <Page size="A4" orientation="portrait">
            <View style={styles.page}>
              <View style={styles.section}>
                <Text style={styles.title}>Min current stock product</Text>
                <Text style={styles.dateText}>Date and time: {formattedDate}</Text>
                <View>
                  {renderTableRow(
                    { description: "Product", stock: "Stock", min_stock: "Min stock" },
                    true
                  )}
